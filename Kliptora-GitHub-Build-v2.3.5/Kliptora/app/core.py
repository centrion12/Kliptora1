...
def find_ffmpeg() -> str | None:
    ...
    # Release builds ship FFmpeg inside the application.  PyInstaller keeps
    # bundled data under ``sys._MEIPASS`` (usually the ``_internal`` folder),
    # so check that location before looking at the per-user repair folder.
    bundled = bundled_resource(f"tools/ffmpeg/bin/{exe}")
    if bundled.exists():
        return str(bundled.parent)

    installed = installation_dir() / "tools" / "ffmpeg" / "bin" / exe
    if installed.exists():
...
def find_deno() -> str | None:
    ...
    bundled = bundled_resource(f"tools/deno/bin/{exe}")
    if bundled.exists():
        return str(bundled.parent)

    installed = installation_dir() / "tools" / "deno" / "bin" / exe
    if installed.exists():
        return str(installed.parent)
