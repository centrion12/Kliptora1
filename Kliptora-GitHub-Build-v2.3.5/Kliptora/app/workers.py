    ...
    def run(self) -> None:
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="kliptora_ffmpeg_") as tmp:
                ...
                with urllib.request.urlopen(request, timeout=90) as response, zip_path.open("wb") as out:
                    status = getattr(response, "status", 200)
                    if status >= 400:
                        raise RuntimeError(f"FFmpeg sunucusu HTTP {status} döndürdü.")
                    ...
                    while True:
                        ...
                        out.write(chunk)
                        received += len(chunk)
                        percent = int(received / total * 75) if total else min(70, 5 + received // (2 * 1024 * 1024))
                        self.progress.emit(min(percent, 75), f"FFmpeg indiriliyor • {received / (1024 * 1024):.1f} MB")
                    out.flush()

                if not zip_path.exists() or zip_path.stat().st_size < 1_000_000:
                    raise RuntimeError("FFmpeg arşivi eksik veya boş indirildi.")
                if total and zip_path.stat().st_size != total:
                    raise RuntimeError("FFmpeg arşivi eksik indi. İnternet bağlantısını kontrol edip tekrar dene.")

                self.progress.emit(80, "FFmpeg arşivi doğrulanıyor")
