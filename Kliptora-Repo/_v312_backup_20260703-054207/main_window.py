...
class MainWindow(QMainWindow):
    ...
    def _build_settings_page(self) -> QWidget:
        ...
        self.install_ffmpeg_button = QPushButton("⇩  FFmpeg'i yeniden kur")
        ...
        self.install_deno_button = QPushButton("⇩  Deno'yu yeniden kur")
    ...
    def _refresh_tool_status(self) -> None:
        ffmpeg = find_ffmpeg()
        deno = find_deno()
        self.ffmpeg_status.setText(f"Hazır: {ffmpeg}" if ffmpeg else "Bulunamadı. Video birleştirme ve dönüştürme için kur.")
        self.deno_status.setText(f"Hazır: {deno}" if deno else "Bulunamadı. Güncel site çözümleme desteği için kur.")
        if ffmpeg and deno:
    ...
    def _install_ffmpeg(self) -> None:
        self._show_tool_notice("FFmpeg arka planda kuruluyor. Uygulamayı kullanmaya devam edebilirsin.", "active")
        ...
        worker.progress.connect(lambda value, text: (self.ffmpeg_progress.setValue(value), self.ffmpeg_status.setText(text)))
    ...
    def _install_deno(self) -> None:
        self._show_tool_notice("Deno arka planda kuruluyor. Pencere kilitlenmeden diğer bölümleri kullanabilirsin.", "active")
        ...
        worker.progress.connect(lambda value, text: (self.deno_progress.setValue(value), self.deno_status.setText(text)))
